...
fontSize:"10px"}});g.addListener("click",function(){k.open(d,g)})}});new google.maps.Marker({position:b,map:d})}function clickThroughToStop(a){refresh();getBusInfo(a);getStopInfo(a);history.pushState&&(a=window.location.protocol+"//"+window.location.host+window.location.pathname+"?stop_id="+a,window.history.pushState({path:a},"",a))}
function getBusInfo(a){$.getJSON("https://bvcors.herokuapp.com/https://data.dublinked.ie/cgi-bin/rtpi/realtimebusinformation?stopid="+a+"&format=json?callback?",function(a){0==a.errorcode?processBusData(a):console.log("failed to retrieve stop info")})}
