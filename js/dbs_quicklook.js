...
try {
    ...
    for (i = 0; i < preferred_stops.length; i++) {
        $("#chips").append(`<span class="mdl-chip chip" onclick="clickThroughToStop(` + preferred_stops[i] + `)"><span class="tooltip mdl-chip__text">` + preferred_stops[i] + `<span class="tooltiptext">Click to see stop info</span></span></span>`)
    }
} catch (err) {
